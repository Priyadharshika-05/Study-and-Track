...
app.post('/api/quiz/generate', requireAuth, async (req, res) => {
  try {
    ...
    if (!nodeIds || !nodeIds.length) return res.status(400).json({ error: 'No topics selected.' });
    if (!process.env.GEMINI_API_KEY) return res.status(500).json({ error: 'Gemini API key not configured.' });

    const allNodes = await Node.find({ userId: req.session.userId });

    function collectDescendants(ids) {
      const result = new Set(ids.map(String));
      let changed = true;
      while (changed) {
        changed = false;
        for (const n of allNodes) {
          if (n.parentId && result.has(String(n.parentId)) && !result.has(String(n._id))) {
            result.add(String(n._id));
            changed = true;
          }
        }
      }
    ...
    const allIds = collectDescendants(nodeIds);
    const selectedNodes = allNodes.filter(n => allIds.has(String(n._id)));

    const contentParts = selectedNodes
      .filter(n => n.content && n.content.replace(/<[^>]+>/g, '').trim())
      .map(n => {
        const title = `${n.numbering ? n.numbering + ' ' : ''}${n.title}`;
        const body = n.content.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
        return `### ${title}\n${body}`;
      });

    if (!contentParts.length) return res.status(400).json({ error: 'Selected topics have no notes yet. Add notes first.' });

    const notesText = contentParts.join('\n\n');
    const numQ = Math.min(Math.max(parseInt(count) || 10, 1), 50);

    const prompt = `You are a strict MCQ generator for TNPSC exam preparation.

Generate exactly ${numQ} multiple choice questions STRICTLY based ONLY on the study notes below. Do not use any outside knowledge. Every question and answer must be directly derivable from these notes only.
...
- Return ONLY a valid JSON array, no markdown fences, no explanation, no extra text whatsoever

Format:
[{"q":"Question?","options":["A. option","B. option","C. option","D. option"],"answer":"A","explanation":"Why this is correct based on the notes."}]
...
${notesText}`;
    ...
    if (!geminiRes.ok) return res.status(500).json({ error: geminiData?.error?.message || 'Gemini API error.' });
    ...
    catch(e) { return res.status(500).json({ error: 'Gemini returned invalid response. Try again.' }); }

    if (!Array.isArray(questions) || !questions.length)
      return res.status(500).json({ error: 'No questions generated. Try again.' });
